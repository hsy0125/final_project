...
SELECT empname ,salary , salary * 1.1 AS "급여 10% 인상"
FROM employee
WHERE title = '과장';
...
WHERE e.dno =d.deptno AND  deptno =3 ;
-- Q25. 모든 사원에 대해서 사원의 이름과 직속 상사의 이름을 검색하시오. (상사가 없는 사원도 포함)
SELECT e.empname, e2.empname AS "직속 상사"
FROM employee e LEFT JOIN employee e2 ON e2.empno = e.manager;
...
FROM employee e , department d 
WHERE e.dno =d.deptno 
ORDER BY d.deptname ASC, salary DESC;
...
-- 전체 사원들의 평균 급여보다 많이 받는 사원들의 이름, 급여를 검색하시오.
SELECT AVG(salary)
FROM employee; 

SELECT empname , salary , ROUND((SELECT AVG(salary) FROM  employee )) AS "평균급여"
FROM employee e 
WHERE salary > (SELECT AVG(salary)
				FROM  employee );
-- Q31. 소속 사원이 한 명도 없는 부서에 대해서 부서 번호, 부서 이름, 층을 검색하시오.
SELECT d.deptno , d.deptname , d.floor 
FROM department d 
WHERE d.deptno NOT IN (SELECT e.dno
FROM employee e)

-- Q32. 조민희가 참여하고 있는 프로젝트의 이름과 예산을 검색하시오.

SELECT p.projname , p.budget
  FROM project p ,works_for w
 WHERE p.projno = w.projno 
   AND w.empno IN (SELECT e.empno
			          FROM employee e 
                     WHERE e.empname = '조민희') ;
-- Q33. 모든 사원의 이름과 부서이름, 그리고 직속상사 이름을 출력하시오.
SELECT e.empname , d.deptname, e2.empname  AS "직속상사"
  FROM department d , employee e LEFT OUTER JOIN employee e2 ON e.manager = e2.empno 
 WHERE e.dno =d.deptno;
-- Q34. 프로젝트 번호 2와 3에 모두 속한 사원의 이름을 출력하시오.
SELECT e.empname 
FROM employee e 
WHERE e.empno IN (SELECT w. empno 
FROM works_for w
WHERE w.projno = 2 )
